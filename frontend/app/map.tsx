...
export default function MapScreen() {
  ...
  const [region, setRegion] = useState({ latitude: 40.758, longitude: -73.9855, latitudeDelta: 0.03, longitudeDelta: 0.03 });
    ...
    return (
      <SafeAreaView style={styles.root}>
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.back()}><Ionicons name="chevron-back" size={26} color={colors.textPrimary} /></TouchableOpacity>
          <Text style={styles.title}>Map</Text><View style={{ width: 26 }} />
        </View>
  ...
  return (
    <View style={styles.root}>
      ...
      <SafeAreaView style={styles.overlay} pointerEvents="box-none">
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.back()} style={styles.headerBtn}><Ionicons name="chevron-back" size={24} color={colors.textPrimary} /></TouchableOpacity>
        ...
        {sel && (
          <TouchableOpacity style={styles.selCard} onPress={() => router.push(`/venue/${sel.id}`)}>
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={styles.selName} numberOfLines={1}>{sel.name}</Text>
              <Text style={styles.selSub}>{sel.kind} · {sel.active_count} live</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 16, backgroundColor: "rgba(3,3,5,0.7)" },
  headerBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: "rgba(0,0,0,0.6)", alignItems: "center", justifyContent: "center" },
  title: { color: colors.textPrimary, fontSize: 16, fontWeight: "800" },
  center: { alignItems: "center", justifyContent: "center" },
  empty: { color: colors.textPrimary, fontSize: 18, fontWeight: "700", marginTop: 16, textAlign: "center" },
  emptySub: { color: colors.textSecondary, marginTop: 8 },
  selCard: { flexDirection: "row", alignItems: "center", padding: 12, margin: 16, backgroundColor: colors.elevated, borderRadius: 18, borderWidth: 1, borderColor: colors.volt },
